...
/*
Next 4 functions are for extra syntax for creating and deleting fd's.
*/
/*
...
/*
return 1 if fd1 create; 2 if fd2 create else 0.
*/
int check_creat_fd(char* qs) {
	...
	if(strcmp(tok_fd, "fd1") == 0) {
		free(tok_creat); free(tok_fd); free(copy_qs);
		return 1;
	}
	else if(strcmp(tok_fd, "fd2") == 0) {
		free(tok_creat); free(tok_fd); free(copy_qs);
		return 2;
	}
